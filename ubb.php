<?
 class UBB{
	...
	function parsefile($fileid,$oldstr) {
		...
		if($fs['extension']=='swf' && $this->isplaymedia){
			$str=$this->parseflash("uploadfile/attachment/{$row['filepath']}");
		}elseif($row['type']==1 && $this->isshowimg && $fs['extension']!='swf' ){
			//图片
	...
	function parsemedia($type, $width, $height, $autostart, $url) {
		if(in_array($type, array('ra', 'rm', 'wma', 'wmv', 'mp3', 'mov','youtube'))) {
			...
			switch($type) {
				...
				case 'youtube'	: $url=str_replace('/watch?v=','/v/',$url);return '<object width="'.$width.'" height="'.$height.'"><param name="movie" value="'.$url.'"></param><param name="allowFullScreen" value="true"></param><param name="allowscriptaccess" value="always"></param><embed src="'.$url.'" type="application/x-shockwave-flash" allowscriptaccess="always" allowfullscreen="true" width="'.$width.'" height="'.$height.'"></embed></object>';break;
			}
		}
		return;
	}
